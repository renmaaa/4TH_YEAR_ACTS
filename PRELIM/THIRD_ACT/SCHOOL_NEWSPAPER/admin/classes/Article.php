...
class Article extends Database {
    public function createArticle($title, $content, $author_id, $image_path = null, $is_active = 1) { 
    ...
    public function getArticles($id = null) {
        if ($id) {
            $sql = "SELECT a.*, spu.username, spu.is_admin FROM articles a JOIN school_publication_users spu ON a.author_id = spu.user_id WHERE a.article_id = ?";
            return $this->executeQuery($sql, [$id])[0] ?? null;
        }
        $sql = "SELECT a.*, spu.username, spu.is_admin FROM articles a JOIN school_publication_users spu ON a.author_id = spu.user_id ORDER BY a.created_at DESC";
        return $this->executeQuery($sql);
    }

    public function getActiveArticles() {
        $sql = "SELECT a.*, spu.username, spu.is_admin FROM articles a JOIN school_publication_users spu ON a.author_id = spu.user_id WHERE a.is_active = 1 ORDER BY a.created_at DESC";
        return $this->executeQuery($sql);
    }
