...
if (isset($_POST['insertNewUserBtn'])) {
    ...
    $is_fiverr_admin = isset($_POST['is_fiverr_admin']) ? 1 : 0; // Check if admin checkbox is set
	...
	if (!empty($username) && !empty($email) && !empty($password) && !empty($confirm_password)) {
		...
		if ($password == $confirm_password) {
			...
			if (!$userObj->usernameExists($username)) {
				...
				if ($userObj->registerUser($username, $email, $password, $contact_number, 1, $is_fiverr_admin)) { // Pass is_client=1 and is_fiverr_admin
	...
	if (!empty($email) && !empty($password)) {
		...
		if ($userObj->loginUser($email, $password)) {
            if ($userObj->isFiverrAdmin()) {
                header("Location: ../client/fiverr_admin/index.php"); // Redirect admin to admin panel
            } else {
                header("Location: ../index.php"); // Redirect regular client to client panel
            }
		}
		else {
...
if (isset($_POST['insertOfferBtn'])) {
    ...
    // Check if an offer already exists for this user and proposal
