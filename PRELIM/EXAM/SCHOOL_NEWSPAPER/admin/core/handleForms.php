...
if (isset($_POST['insertAdminArticleBtn'])) {
    ...
    $category_id = $_POST['category_id'] ?? null; // Get category ID

    if ($title !== '' && $content !== '') {
        if ($articleObj->createArticle($title, $content, $author_id, null, 1, $category_id)) { // Pass category_id
...
if (isset($_POST['editArticleBtn'])) {
    ...
    if ($article_id && $title !== '' && $content !== '') {
        if ($articleObj->updateArticle($article_id, $title, $content, null, $category_id)) { // Pass category_id
...
if (isset($_POST['insertArticleBtn'])) {
    ...
    if ($title !== '' && $description !== '') {
        $is_active = 0;
        if ($articleObj->createArticle($title, $description, $author_id, $image_path, $is_active, $category_id)) { // Pass category_id
...
if (isset($_POST['createCategoryBtn'])) {
    ...
    if (!empty($category_name)) {
        if ($articleObj->createCategory($category_name)) {
            ...
            $_SESSION['message'] = "Failed to create category or category already exists.";
            $_SESSION['status'] = '400';
        }
    } else {
...
if (isset($_POST['updateCategoryBtn'])) {
    ...
    if ($category_id && !empty($category_name)) {
        if ($articleObj->updateCategory($category_id, $category_name)) {
            $_SESSION['message'] = "Category updated successfully!";
            $_SESSION['status'] = '200';
        } else {
            $_SESSION['message'] = "Failed to update category or category name already exists.";
            $_SESSION['status'] = '400';
        }
    } else {
        $_SESSION['message'] = "Invalid data for category update.";
        $_SESSION['status'] = '400';
    }
    header("Location: ../admin/categories.php");
    exit();
...
if (isset($_POST['deleteCategoryBtn'])) {
    ...
    $category_id = $_POST['category_id'] ?? null;
    if ($category_id) {
        if ($articleObj->deleteCategory($category_id)) {
