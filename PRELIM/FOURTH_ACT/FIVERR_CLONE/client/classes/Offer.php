...
class Offer extends Database {
    ...
    /**
     * Checks if an offer already exists for a given user and proposal.
     * @param int $user_id The ID of the user.
     * @param int $proposal_id The ID of the proposal.
     * @return bool True if an offer exists, false otherwise.
     */
    public function offerExists($user_id, $proposal_id) {
        $sql = "SELECT COUNT(*) as offer_count FROM offers WHERE user_id = ? AND proposal_id = ?";
        $count = $this->executeQuerySingle($sql, [$user_id, $proposal_id]);
        return $count['offer_count'] > 0;
    }
